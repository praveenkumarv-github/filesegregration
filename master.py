...
def imageSegregration() :  
    ...
    if res_jpg  :
        src = filePath
        des = r"K:\\miFilesegregration\\jpg\\"   ####Destination Path  
        shutil.copy2(filePath,des)
        print(("1 --- {} moved  to {}").format(filePath,des))
    if res_png :
        src = filePath
        des = r"K:\\miFilesegregration\\png\\"      ####Destination Path  
        shutil.copy2(filePath,des)
        print(("2--- {} moved to {}").format(filePath,des))
    if res_gif  :
        src = filePath
        des = r"K:\\miFilesegregration\\gif\\"      ####Destination Path  
        shutil.copy2(filePath,des)
        print(("3 --- {} moved  to {}").format(filePath.des))
    if res_mp4  :
        src = filePath
        des = r"K:\\realme_phone bachup_dec29_2019\\segregation\\videos\\"      ####Destination Path  
        shutil.copy2(filePath,des)
        print(("3 --- {} moved  to des").format(filePath))
    if res_py  :
        src = filePath
        des = r"K:\\realme_phone bachup_dec29_2019\\segregation\\py\\"          ####Destination Path  
        shutil.copy2(filePath,des)
        print(("3 --- {} moved  to des").format(filePath))
